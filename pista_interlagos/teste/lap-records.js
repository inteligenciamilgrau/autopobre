...
export class LapRecords {
 ...
 open(mode=null){
  ...
  if(this.candidate){$('recordsMode').value=this.candidate.mode;$('recordCandidate').textContent=`${CIRCUITS[this.candidate.circuit].name} · Sua volta: ${formatTime(this.candidate.bestLap)} · Corrida: ${formatTime(this.candidate.bestRace)}`;try{$('recordName').value=localStorage.getItem('autopobre-record-name')||'';}catch{}}
  this.render();this.dialog.showModal();
 }
 render(){
  let rows=[];try{rows=readRecords(localStorage);}catch{}
  const mode=this.dialog.querySelector('#recordsMode').value;rows=rows.filter(r=>r.mode===mode&&r.circuit===this.dialog.querySelector('#recordsCircuit').value).sort((a,b)=>a.bestLap-b.bestLap);
