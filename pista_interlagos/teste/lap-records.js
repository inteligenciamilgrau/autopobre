...
export function saveRecord(storage,{name,mode,bestLap,bestRace}){
 ...
 const rows=readRecords(storage),existing=rows.find(r=>r.mode===mode&&r.name.toLocaleLowerCase('pt-BR')===name.toLocaleLowerCase('pt-BR'));
 ...
 else rows.push({name,mode,bestLap,bestRace,date:new Date().toISOString()});
 const sorted=rows.sort((a,b)=>a.bestLap-b.bestLap).slice(0,50);
 if(!sorted.some(r=>r.mode===mode&&r.name.toLocaleLowerCase('pt-BR')===name.toLocaleLowerCase('pt-BR')))throw new Error('A lista guarda os 50 melhores recordes. Esse tempo ficou fora da lista.');
...
export class LapRecords {
 ...
 open(mode=null){
  const $=id=>this.dialog.querySelector('#'+id);this.candidate=mode&&!mode.recordAssisted&&validTime(mode.finishBest)&&validTime(mode.finishTime)?{mode:mode.active?'immersive':'normal',bestLap:mode.finishBest,bestRace:mode.finishTime}:null;
  ...
  if(this.candidate){$('recordsMode').value=this.candidate.mode;$('recordCandidate').textContent=`Sua volta: ${formatTime(this.candidate.bestLap)} · Corrida: ${formatTime(this.candidate.bestRace)}`;try{$('recordName').value=localStorage.getItem('autopobre-record-name')||'';}catch{}}
  this.render();this.dialog.showModal();
 }
 render(){
  let rows=[];try{rows=readRecords(localStorage);}catch{}
  const mode=this.dialog.querySelector('#recordsMode').value;rows=rows.filter(r=>r.mode===mode).sort((a,b)=>a.bestLap-b.bestLap);
