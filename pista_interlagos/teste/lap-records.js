...
export class LapRecords {
 ...
 render(){
  ...
  this.dialog.querySelector('#recordsSourceNote').textContent=this.selectedSource==='human'?'Somente pessoas que jogaram neste navegador.':'IA: tempos simulados no jogo, usados como metas. Não são recordes reais dos pilotos.';
  ...
  rows.forEach((row,i)=>{const tr=document.createElement('tr');tr.dataset.source=row.source;for(const value of [`${i+1}º`,row.name,formatTime(row.bestLap),formatTime(row.bestRace)]){const td=document.createElement('td');td.textContent=value;tr.append(td);}const badge=document.createElement('span');badge.className='record-source-badge';badge.textContent=row.source==='ai'?`IA · #${row.number}`:'PESSOA';tr.children[1].append(badge);body.append(tr);});this.dialog.querySelector('#recordsEmpty').hidden=rows.length>0;
 }
}
