...
export class PitStop {
 ...
 render(){
  ...
  if(walking){
   ...
   hud('pitWalkJob').textContent=busy?`Mecânico: ${jobName(job)} · ${Math.ceil(job.seconds-job.elapsed)} s`+(this.service.queue.length?` · +${this.service.queue.length} na fila`:''):'Opala pronto. Volte quando quiser.';
