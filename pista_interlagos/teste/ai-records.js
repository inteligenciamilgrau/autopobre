...
export function readAIRecords(storage){
 ...
 try{const saved=JSON.parse(storage?.getItem(KEY)||'[]');if(Array.isArray(saved))for(const r of saved){const row=records.get(key(r||{}));if(row&&valid(r.bestLap)&&(r.bestRace===null||valid(r.bestRace)&&r.bestRace>=r.bestLap)){row.bestLap=Math.min(row.bestLap,r.bestLap);row.bestRace=best(row.bestRace,r.bestRace);}}}catch{}
...
export class AutomaticAIRecords {
 ...
 update(mode,now=Date.now()){
  ...
  const circuit=mode.data.meta.id,category=mode.active?'immersive':'normal';
  ...
  for(const rival of mode.rivals){
   ...
   const race=rival.finished&&valid(rival.finishTime)?rival.finishTime:null;
  ...
  if(changed){try{this.storage.setItem(KEY,JSON.stringify(rows.map(({circuit,mode,number,bestLap,bestRace})=>({circuit,mode,number,bestLap,bestRace}))));this.rows=rows;this.retryAt=0;}catch{this.retryAt=now+5000;return;}}
