...
export function buildRearCabin(kit){
 const {root,mesh,box,bar,m,chrome,paintBoth,cabin,classic}=kit;
 // --- Floor: footwells, tunnel hump, heel riser, bare seat pan, bulkhead and shelf.
 box([-.80,.29,0],[.34,.04,1.49],m.paint,cabin);
 // Driveshaft hump from under the seats to the heel riser (the gearbox tunnel is in cockpit.js).
 mesh(kit.tunnel(.165,.78),m.paint,[-.20,.31,0],cabin);
 box([-.975,.385,0],[.012,.19,1.46],m.paint,cabin);
 box([-1.18,.47,0],[.41,.02,1.46],m.paint,cabin);
 for(const x of [-1.06,-1.28])box([x,.482,0],[.05,.006,1.2],m.paint,cabin); // pressed ribs of the pan
 ...
 }),transparent:true,alphaTest:.05,roughness:.85,polygonOffset:true,polygonOffsetFactor:-2}),[-1.06,.4803,.40],cabin);
 rust.rotation.x=-Math.PI/2;rust.castShadow=false;
 box([-1.418,.735,0],[.012,.515,1.46],m.paint,cabin).rotation.z=.127;
 box([-1.495,.992,0],[.11,.012,1.40],m.paint,cabin);
 for(const side of [-1,1]){
  box([-1.10,.585,side*.745],[.94,.59,.02],m.paint,cabin);
  box([-1.10,.885,side*.753],[.94,.018,.045],m.body,cabin);
  // Rear wheel tubs beside the pan, arched over the 1.117 m axle.
  mesh(new THREE.SphereGeometry(1,32,12,0,Math.PI*2,0,Math.PI/2).scale(.34,.27,.12),kit.paintSatin,[-1.117,.31,side*.745],cabin);
  // Roof rail over the door and quarter window, and the C-pillar down to the belt.
  box([-.28,1.365,side*.725],[1.16,.06,.13],m.paint,classic);
  const s=side,g=REAR_GLASS;
  mesh(patch([[g.top,g.topY,s*g.topHalf],[-.86,1.335,s*.70],[-.84,1.285,s*.78],[-.93,1.20,s*.78],[-1.07,.895,s*.775],[-1.56,.885,s*.765],[g.bottom,g.bottomY,s*g.bottomHalf]]),paintBoth,[0,0,0],classic);
  ...
  mesh(new THREE.ShapeGeometry(shape),m.tint,[0,0,side*.782],classic);
  ...
  for(const [x,y,w,h,round] of [[-.52,1.0,.06,.06,true],[-.60,.96,.05,.05,true],[-.53,1.09,.07,.035,false],[-.66,1.04,.055,.055,false],[-.72,.95,.045,.045,true],[-.62,1.15,.04,.04,true]]){
   const o=mesh(round?new THREE.CircleGeometry(w/2,24):new THREE.PlaneGeometry(w,h),stickerBack,[x,y,side*.779],classic);o.castShadow=false;
  }
 }
 ...
 const rearGlass=mesh(glass,new THREE.MeshPhysicalMaterial({map:rearWindowTexture(),transparent:true,depthWrite:false,side:THREE.DoubleSide,roughness:.06,clearcoat:.4,envMapIntensity:.3}),[0,0,0],classic);rearGlass.castShadow=false;
 ...
 const sticker=mesh(new THREE.PlaneGeometry(.10,.09),new THREE.MeshBasicMaterial({map:dSticker(),transparent:true,alphaTest:.1}),at(.083,.87).toArray(),classic);
 ...
 box([-1.84,.955,0],[.62,.04,1.56],m.hood,classic).rotation.z=.055;
 const rtj=mesh(new THREE.PlaneGeometry(.42,.13),new THREE.MeshStandardMaterial({map:rtjDecal(),transparent:true,alphaTest:.1,roughness:.5,polygonOffset:true,polygonOffsetFactor:-2}),[-2.02,.9665,0],classic);
 ...
 for(const side of [-1,1]){
  bar([-.63,1.36,side*.59],[-1.62,.85,side*.58],.022,m.cage,classic);
  bar([-.63,.60,side*.64],[-1.40,.60,side*.70],.02,m.cage,classic);
 }
 bar([-1.35,.99,-.585],[-1.35,.99,.585],.02,m.cage,classic);
 bar([-.96,.60,-.666],[-.96,.60,.666],.02,m.cage,classic);
 // --- Battery in the passenger footwell under a clamp bar (photo 34). Its parts gather in one
 // group (named for cockpit.js setView, which brings it inboard in the V06 body).
 const beforeBattery=root.children.length;
 ...
 box([-.70,.355,.62],[.09,.09,.08],m.shell,classic); // small black box beside the battery (inside the V06 rear arch)
 const cable=(points,r=.007,parent)=>mesh(new THREE.TubeGeometry(new THREE.CatmullRomCurve3(points.map(p=>new THREE.Vector3(...p))),40,r,8,false),m.plastic,[0,0,0],parent);
 cable([[-.745,.515,.51],[-.70,.52,.56],[-.64,.34,.62],[-.2,.325,.66],[.3,.325,.60],[.62,.33,.52]],undefined,classic); // runs in the V06 sill
 cable([[-.745,.515,.33],[-.70,.50,.28],[-.70,.33,.24],[-.95,.32,.20]],.006);
 const battery=new THREE.Group();battery.name='Bateria';for(const o of root.children.slice(beforeBattery))battery.add(o);root.add(battery);
