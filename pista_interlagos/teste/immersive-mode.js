...
export class ImmersiveMode {
 ...
 handleKey(code){
  ...
  if(s.phase==='crowd'){
   ...
   // E is the action key, GTA style: by the Opala it lifts the hood (at the nose), the trunk lid
   // (at the tail) or gets in (at the driver's door); elsewhere it talks to the nearest supporter.
   if(code==='KeyE'){this.action(s.fan!==null?'close':this.visual.carAction()?'car':'talk');used=true;}
 ...
 ui(){
  ...
  document.getElementById('immAlert').textContent=s.phase==='crowd'?(this.inCar?'F: sair do Opala · o mouse olha em volta':s.fan===null?this.carHint():''):s.phase==='race'?(s.alertTime>0?s.alert:s.tankDetached?'Tanque solto · combustível vazando':s.fuel<1?'Reserva! A gasolina está acabando.':'Guarde distância: o carro da frente pode soltar peças.'):s.phase==='tow'?(s.towGap<3?'FREIE · FITA FROUXA':'Controle o freio quando o caminhão diminuir.'):'';
  const spot=s.phase==='crowd'?this.visual.carAction():null,key=[s.phase,s.revision,this.near,s.fan,spot,spot&&this.visual.ownOpen(spot)].join(':');
