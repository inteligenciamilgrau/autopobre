...
with sync_playwright() as p:
 ...
 try:
  ...
  page.set_viewport_size({'width':1440,'height':900});page.click('#settingsBack');enter_track(page,story=True)
  ...
  page.evaluate('''async()=>{const {FANS,JOKES}=await import('./immersive-state.js');const m=fixtureMode,s=m.state;for(let i=0;i<FANS.length;i++){s.talk(i);s.joke(JOKES.findIndex(j=>j.topic===FANS[i].taste));}m.action('close');m.action('desk');m.action('buy');}''');wait_js(page,"interlagos.immersiveInfo().phase==='starting'")
  page.wait_for_timeout(400);page.screenshot(path=str(ROOT/'renders/grid_partida_cockpit.png'))
  check('engine_start_in_cockpit_with_revving_grid',page.evaluate('''()=>{const m=fixtureMode;return interlagos.state.mode==='cockpit'&&m.visual.rivals.every(x=>x.visible)&&interlagos.audioInfo().effects.loops.rival0>0;}'''))
  page.click('#menuButton');page.click('#settingsBack');enter_track(page)
