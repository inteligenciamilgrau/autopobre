...
with sync_playwright() as p:
 ...
 try:
  ...
  check('opposite_arms_and_legs',page.evaluate('''()=>{const v=fixtureMode.visual;v.walkCycle=.7;v.walk({throttle:1,brake:0,left:0,right:0},.06);const a=v.hero.userData.limbs.map(x=>x.rotation.z);return a[0]*a[2]<0&&a[1]*a[3]<0&&a[0]*a[3]>0&&a[1]*a[2]>0;}'''))
