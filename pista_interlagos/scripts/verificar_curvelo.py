...
with sync_playwright() as p:
 ...
 for mobile in ([True] if '--mobile' in sys.argv else [False,True]):
  ...
  page.goto('http://127.0.0.1:8799/pista_interlagos/teste/',wait_until='domcontentloaded');wait_js(page,'window.interlagos?.ready')
  ...
  page.click('#start');assert page.evaluate("!interlagos.state.paused&&interlagos.state.mode==='chase'")
  ...
  page.wait_for_url('**/*circuito=interlagos');wait_js(page,'window.interlagos?.ready')
