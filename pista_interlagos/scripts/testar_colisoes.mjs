...
const data=JSON.parse(fs.readFileSync(new URL('../dados/pista.json',import.meta.url))),player=new TestCar(data),field=new RaceField(data);
for(let i=0;i<120*25;i++)field.step(player,1/120,false);
assert.equal(field.rivals.length,5);assert(field.rivals.every(r=>r.progress>100&&Number.isFinite(r.car.heading)));assert(field.rivals.every(r=>Math.abs(r.car.surface.d)<r.car.surface.width/2+1));
for(let i=0;i<5;i++)for(let j=i+1;j<5;j++){const hit=bodyContact(field.rivals[i].car,field.rivals[j].car);assert(!hit||hit.depth<.02);}
const challenger=new TestCar(data),fast=new RaceField(data);let maxOffroad=0;
...
console.log('Collision tests passed: rotated bodies, side/corner/head-on contact, momentum, high closing speed, five AI cars and 25 seconds of traffic.');
