...
with sync_playwright() as p:
 ...
 try:
  ...
  for selector in ['#touchSteering','[data-key="KeyW"]','[data-key="KeyS"]']:
   rect=page.locator(selector).bounding_box();stray={'x':rect['x']+rect['width']+12,'y':rect['y']+rect['height']/2,'id':8}
   check(f'near_control_test_hits_canvas_{selector}',page.evaluate('p=>document.elementFromPoint(p.x,p.y).id',stray)=='view')
   ...
   cdp.send('Input.dispatchTouchEvent',{'type':'touchEnd','touchPoints':[]});check(f'near_control_drag_does_not_move_camera_{selector}',page.evaluate("interlagos.state.mode==='chase'"))
