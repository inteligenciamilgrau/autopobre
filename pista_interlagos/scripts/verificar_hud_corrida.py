...
with sync_playwright() as p:
 ...
 try:
  ...
  finger={'x':center+20,'y':steer['y']+steer['height']/2,'id':2};cdp.send('Input.dispatchTouchEvent',{'type':'touchStart','touchPoints':[pedal,finger]});wait_js(page,"interlagos.mobileInfo().steering>.04&&interlagos.mobileInfo().pressed.includes('KeyW')");check('gentle_steering_near_center_with_gas',page.evaluate('interlagos.mobileInfo().steering')<.12)
  finger['x']=center+(steer['width']/2-22)*.5;cdp.send('Input.dispatchTouchEvent',{'type':'touchMove','touchPoints':[pedal,finger]});check('half_slider_gives_about_quarter_steering',.2<page.evaluate('interlagos.mobileInfo().steering')<.26)
