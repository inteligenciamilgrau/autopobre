...
assert.deepEqual(preferences.values,{immersive:true,livery:'assinaturas_omp',camera:'chase'});
preferences.update({immersive:false,livery:'seiva_danilo',camera:'cockpit'});
preferences=new PlayerPreferences(storage);
assert.deepEqual(preferences.values,{immersive:false,livery:'seiva_danilo',camera:'cockpit'});
...
for(const corrupted of ['not json','null','[]','42','{"immersive":"false","livery":"../../private","camera":"bad"}']){
 ...
 assert.deepEqual(new PlayerPreferences(storage).values,{immersive:true,livery:'assinaturas_omp',camera:'chase'});
}
