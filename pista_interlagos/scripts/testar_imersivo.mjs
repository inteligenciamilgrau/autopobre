...
const goodTow=fresh();goodTow.beginTow();for(let i=0;i<12000&&goodTow.phase==='tow';i++)goodTow.towStep({brake:1},1/120);check('braking_keeps_tow_strap_safe',goodTow.phase==='inspection'&&goodTow.towSnags===0);
...
const winner=fresh();winner.phase='race';winner.finish(1);winner.requestInspection();winner.inspectionStep(8);check('winner_still_sixth_on_podium',winner.inspected&&winner.podiumPlace===6&&winner.result.position===1&&winner.prize===600);
...
const dq=fresh();dq.phase='race';dq.finish(1);dq.goToBox();check('box_before_judge_disqualifies_winner',dq.result.status==='Desclassificado'&&dq.prize===0&&dq.podiumPlace===6);
for(const reason of ['Motor afogado na partida','Acabou a gasolina','Para-brisa estilhaçado','Carro quebrou']){const d=fresh();d.phase='race';d.fail(reason);d.beginTow();while(d.phase==='tow')d.towStep({brake:1},.05);d.requestInspection();d.inspectionStep(8);check('dnf_sixth_'+reason,d.podiumPlace===6&&d.result.status==='Não terminou');}
winner.start();winner.phase='race';winner.finish(1);winner.requestInspection();winner.inspectionStep(8);check('blazer_goal_can_be_completed',winner.releaseBlazer()&&winner.profile.released&&winner.profile.fund===300);check('mechanic_paid_once',!winner.releaseBlazer());
